/**
* Name: Parte6
* Objetivo: Enviar mensajes a coches cercanos para que cambien el camino cuando hay 30 coches en fila en una intersección
* Author: Lili
* Tags: 
*/
...
global {
	...
	init {
		...
		// Usar la distancia como peso
		//edge_weights 		<- road as_map (each::each.shape.perimeter);
		
		// Usar la congestión en el camino como peso
		edge_weights 		<- road as_map (each::length(each.all_agents));
	...
	// Revisar los 5 caminos con mayor congestión
	reflex check_road when:every(60#cycles)
	{
		int count <- 1;
...
species vehicle_sim parent: base_vehicle {
	...
	reflex recompute_path_reflex when: recompute_path
	{
		edge_weights 		<- road as_map (each::length(each.all_agents));
		road_network 		<- as_driving_graph(road, intersection_recolector) with_weights edge_weights;
...
species car parent: vehicle_sim {
	...
	reflex send_messages_near_vehicles when:current_road != nil and length(road(current_road).all_agents)>=30 and (current_road != nil and road_on != road(current_road))
	{
...
experiment city type: gui {
	...
	output synchronized: true {
		...
		display Statistics	
		{
			chart "Pollution " background:#white type: scatter
			{  //size: {0.5,0.5} position: {0.5, 0}  {
	        	data "Pollution" value: avg_pollution accumulate_values: true line_visible:true marker:false;
			}
		}
